#!/usr/bin/env python3
"""Benchmark assignment analysis queries on MongoDB."""
...
from analyze_mongodb import build_keywords_from_q2, run_q1, run_q2, run_q3


REPO_ROOT = Path(__file__).resolve().parents[1]
RESULTS_DIR = REPO_ROOT / "benchmark_results"
RESULTS_FILE = RESULTS_DIR / "mongo_results.csv"
DATABASE_LABEL = "mongo"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Benchmark MongoDB analysis queries.")
...
def main() -> None:
    ...
    print(f"Benchmarking MongoDB: {db_name} @ {mongo_uri}")
    ...
    with MongoClient(mongo_uri, serverSelectionTimeoutMS=10000) as client:
        ...
        q1_rows, _, _ = benchmark_query("q1", runs, lambda: run_q1(db), DATABASE_LABEL)
        all_rows.extend(q1_rows)

        q2_rows, _, q2_result_rows = benchmark_query("q2", runs, lambda: run_q2(db), DATABASE_LABEL)
        ...
        q3_rows, _, _ = benchmark_query("q3", runs, lambda: run_q3(db, keywords), DATABASE_LABEL)
